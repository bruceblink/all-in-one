...
一个原子由两部分组成：文本和值。原子文本是您放在冒号后面的任何内容。在运行时，此文本保存在原子表中。值是放入变量中的数据，它仅仅是对原子表的一个引用。
这正是原子最适合用作命名常量的原因。它们在内存和性能方面都很高效。当您写下
...
***别名（ALIASES）***
原子常量还有另一种语法。您可以省略开头的冒号，而以大写字母开头：
...
当您使用别名时，编译器会隐式地将 Elixir. 前缀添加到其文本中并生成原子。但如果别名已经包含了 Elixir. 前缀，则不会再添加。
因此，以下操作也同样有效：
...
***原子作为布尔值（ATOMS AS BOOLEANS）***
Elixir 没有专门的布尔类型，这可能会让人感到惊讶。相反，它使用原子 :true 和 :false。作为语法糖，Elixir 允许您在不带起始冒号的情况下引用这些原子：
...
***Nil 与真值（NIL AND TRUTHY VALUES）***
另一个特殊的原子是 :nil，它的工作方式与其他语言中的 null 有些类似。您可以不带冒号引用 nil：
