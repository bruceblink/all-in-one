...
> **比较并交换循环的优化**
>
> 正如我们在第135页的"x86比较并交换指令"中看到的，`fetch_or` 操作和等效的 `compare_exchange` 循环在x86-64上编译为完全相同的指令。人们可能期望在ARM上也会发生同样的情况，至少对于 `compare_exchange_weak`，因为加载和弱比较并交换操作可以直接映射到LL/SC指令。
>
