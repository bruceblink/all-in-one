...
本文档用于描述一个面向应用的、分布式的运行时操作系统（以下简称 D-AROS）的总体设计思想、核心目标、架构划分与关键机制。

---

## 文档定位

本文档描述一个以**应用（Application）**为中心的分布式运行时操作系统 —— **D-AROS（Distributed Application Runtime Operating System）** 的设计目标、责任边界、核心抽象与总体架构，用于指导工程实现、系统评审与长期演进。
...
**Application** 表示一个完整的业务运行单元，是系统中的一等公民，通常包含：

- 多个组件（Component / Workload）
- 全局资源与配额约束
- SLA / QoS 目标
- 生命周期与发布策略
...
### 4.2 Resource Pool（资源池）

**Resource Pool** 是对底层资源的逻辑抽象，包括：

- CPU / Memory
- Storage
- GPU / FPGA 等专用设备

资源池屏蔽节点细节，仅提供**可裁决的容量视图**。
...
系统以**状态迁移（State Transition）**作为核心控制手段，而非直接操纵底层实例。
